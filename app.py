...
st.sidebar.header("📦 4. 测量电缆与补偿")
cable_l = st.sidebar.number_input("测量线长度 (m)", value=20.00, step=0.01, format="%.2f")
z0_cable = st.sidebar.number_input("电缆阻抗 Z0 (Ω)", value=50.00, step=0.01, format="%.2f")
...
try:
    ...
    tr_div_ns = (t_v[idx90] - t_v[idx10]) * 1e9
    tr_total_ns = np.sqrt(tr_div_ns**2 + tr_src_ns**2)
    c1.metric("合成 Tr", f"{tr_total_ns:.2f} ns")
except:
    c1.metric("合成 Tr", "N/A")
