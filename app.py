...
r1_val = st.sidebar.number_input("高压臂电阻 R1 (Ω)", value=r1_init, step=0.01, format="%.2f")
r2_val = st.sidebar.number_input("低压臂电阻 R2 (Ω)", value=10.00, step=0.01, format="%.2f")
...
rs_val = st.sidebar.number_input("首端匹配电阻 Rs (Ω)", value=50.00, step=0.01, format="%.2f")
z0_val = st.sidebar.number_input("电缆特性阻抗 Z0 (Ω)", value=50.00, step=0.01, format="%.2f")
rt_val = st.sidebar.selectbox("示波器输入阻抗 (Ω)", [1000000.0, 50.0], index=1)

# --- 3. 自动化物理建模计算 ---
def run_auto_physics_sim():
    # A. 寄生电容 Cg 自动计算 (对地耦合模型)
    eps0 = 8.854187e-12
    # 物理模型：Cg 主要受高度和均压环直径影响
    cg_pf = (2 * np.pi * eps0 * h_total) / (np.log(4 * h_total / d_ring) - 1) * 1e12 * 1.15
    
    # B. 全回路电感 L 自动计算 (引线 + 分压器本体)
    # 考虑高压引线 Lw 和 分压器高度 H 形成的回路面积
    mu0 = 4 * np.pi * 1e-7
    path_len = l_wire + h_total
    eff_r = d_ring / 2
    # 电感估算公式 (uH)
    l_loop_uh = 0.2 * path_len * (np.log(2 * path_len / eff_r) - 0.75)
    
    # C. 汇总系统参数 (统一单位)
    L_sys = l_loop_uh * 1e-6
    C_sys = cg_pf * 1e-12
    # 总阻尼 = 引线阻尼 + 首端匹配 + (若示波器50欧则包含电缆阻抗)
    r_total_sys = r_damp + rs_val + (z0_val if rt_val == 50.0 else 0)
    ...
    # E. 实际测量变比 K (考虑负载效应修正)
    r2_eff = (r2_val * rt_val) / (r2_val + rt_val)
    k_actual = (r1_val + r2_eff) / r2_eff
...
c2.metric("自动计算 Cg", f"{cg_res:.2f} pF")
c3.metric("自动回路 L", f"{l_res:.2f} μH")
...
st.info(f"💡 自动建模逻辑：回路电感已根据引线长度 {l_wire:.2f}m 和设备高度 {h_total:.2f}m 的几何包围面积自动推导。")
